 ...
 * 1. Test Create product flow (0.5 điểm)
 * 2. Test Read/List products (0.5 điểm)
 * 3. Test Update product (0.5 điểm)
 * 4. Test Delete product (0.5 điểm)
 * 5. Test Search/Filter functionality (0.5 điểm)
 */
...
describe('Product Management - E2E Test Scenarios (2.5 điểm)', () => {
  ...
  // ============================================================
  // SCENARIO 1: Test Create Product Flow (0.5 điểm)
  // ============================================================
  describe('Scenario 1: Test Create product flow (0.5 điểm)', () => {
    ...
    it('SC1.2: Should display all required form fields', () => {
      ...
      ProductPage.productNameInput.should('be.visible').and('have.attr', 'placeholder');
      ProductPage.priceInput.should('be.visible').and('have.attr', 'placeholder');
      ProductPage.quantityInput.should('be.visible').and('have.attr', 'placeholder');
      ProductPage.categorySelect.should('be.visible');
      ProductPage.descriptionInput.should('be.visible').and('have.attr', 'placeholder');
    });
      ...
      ProductPage.tableRows.each(($row) => {
        ...
        cy.wrap($row).within(() => {
          cy.get('td').each(($cell, index) => {
            if (index < 4) { // First 4 columns are required data
  ...
  // ============================================================
  // SCENARIO 3: Test Update Product (0.5 điểm)
  // ============================================================
  describe('Scenario 3: Test Update product (0.5 điểm)', () => {
    ...
    it('SC3.10: Should update multiple fields simultaneously', () => {
      ...
      ProductPage.descriptionInput.clear().type('Mô tả sản phẩm đã được cập nhật');
      ProductPage.submitForm();
  ...
  describe('Scenario 4: Test Delete product (0.5 điểm)', () => {
    ...
    it('SC4.2: Should show confirmation message with product details', () => {
      ...
      cy.get('.modal-small').should('be.visible');
      cy.contains(/bạn có chắc chắn muốn xóa|are you sure/i).should('be.visible');
    });

    it('SC4.3: Should successfully delete product when confirming', () => {
      // Given: Delete confirmation dialog is open
      const firstProductName = cy.get('.product-table tbody tr').first().find('td').eq(0).invoke('text');
  ...
  describe('Scenario 5: Test Search/Filter functionality (0.5 điểm)', () => {
    ...
    it('SC5.14: Should filter by each available category', () => {
      ...
      categories.forEach(category => {
        ...
        cy.get('.product-table tbody tr').then($rows => {
          if (!$rows.text().includes('Không có sản phẩm')) {
            cy.get('.product-table tbody tr').should('have.length.greaterThan', 0);
          }
        });
      });
    });
